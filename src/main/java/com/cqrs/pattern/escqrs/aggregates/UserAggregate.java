package com.cqrs.pattern.escqrs.aggregates;
...
import com.cqrs.pattern.es.service.UserUtility;
...
public class UserAggregate {
	...
	public List<Event> handleUpdateUserCommand(UpdateUserCommand command) {
		...
		List<Contact> contactsToRemove = user.getContacts().stream().filter(c -> !command.getContacts().contains(c))
		...
		List<Contact> contactsToAdd = command.getContacts().stream().filter(c -> !user.getContacts().contains(c))
				.collect(Collectors.toList());
		...
		List<Address> addressesToRemove = user.getAddresses().stream().filter(a -> !command.getAddresses().contains(a))
				.collect(Collectors.toList());
		...
		List<Address> addressesToAdd = command.getAddresses().stream().filter(a -> !user.getAddresses().contains(a))
				.collect(Collectors.toList());
